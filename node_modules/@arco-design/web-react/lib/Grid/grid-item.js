...
function GridItem(baseProps, ref) {
    ...
    var children = props.children, className = props.className, style = props.style, propOffset = props.offset, propSpan = props.span, computedIndex = props.index;
    ...
    return (react_1.default.createElement("div", { ref: ref, className: classNames, style: __assign(__assign({}, gridItemStyle), style) }, react_1.default.Children.map(children, function (child) {
        if (child) {
            var childProps = __assign({ overflow: overflow }, child.props);
            return child.type ? react_1.default.cloneElement(child, childProps) : child;
        }
        return null;
    })));
}
